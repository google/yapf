    ...
    def Visit_trailer( self, node ):                                                   # pylint: disable=invalid-name
        ...
        if node.children[ 0 ].value == '.':
            ...
            if name in { 'argument', 'comparison' }:
                ...
                _SetStronglyConnected( node.children[ 1 ] )
                if ( len( node.children[ 1 ].children ) > 1 and pytree_utils.NodeName(
                ...
                if split_before:
                    _SetSplitPenalty(
                        pytree_utils.LastLeafNode( node.children[ 1 ].children[ 1 ] ),
                        0 )
                else:
                    _SetSplitPenalty(
                        pytree_utils.FirstLeafNode( node.children[ 1 ].children[ 2 ] ),
                        0 )
