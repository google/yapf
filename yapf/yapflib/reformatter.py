            ...
            if ( tok.is_comment and isinstance( tok.spaces_required_before, list ) and
                ...
                while True:
                    ...
                    for line_tok in this_line.tokens:
                        ...
                        if newline_index != -1:
                            max_line_length = max(
                                max_line_length, len( line_content ) )
                            line_content = ''
                ...
                for all_pc_line_lengths_index, pc_line_lengths in enumerate(
                    ...
                    for line_tok in this_line.tokens:
                        if line_tok.is_comment:
                            ...
                            whitespace = ' ' * (
                                aligned_col - pc_line_lengths[ pc_line_length_index ] -
                                1 )
        ...
        for tok in line.tokens:
            if tok.is_assign or tok.is_augassign:
                ...
                while True:
                    ...
                    this_line_index = final_lines_index + len(
                        all_pa_variables_lengths )
                    ...
                    for index in range( len( line_tokens ) ):
                        ...
                        if line_tok.is_assign or line_tok.is_augassign:
                            next_toks = [
                                line_tokens[ i ]
                                for i in range( index + 1, len( line_tokens ) )
                            ]
                            ...
                            for tok in next_toks:
                                if tok.value in [ '(', '[', '{' ] and tok.next_token:
                                    if (
                                            tok.next_token.formatted_whitespace_prefix
                                            .startswith( '\n' ) or
                                        ( tok.next_token.is_comment and
                                          tok.next_token.next_token.
                                          formatted_whitespace_prefix.startswith( '\n' )
                                        ) ):
                                        pa_variables_lengths.append(
                                            len( variables_content ) )
                                        contain_object = True
                                        break
                    ...
                    if pa_variables_lengths:
                        max_variables_length = max(
                            max_variables_length, max( pa_variables_lengths ) )
                    ...
                    for line_tok in this_line.tokens:
                        if line_tok.is_assign or line_tok.is_augassign:
                            ...
                            if pa_variables_lengths_index < len( pa_variables_lengths ):
                                whitespace = ' ' * (
                                    max_variables_length - pa_variables_lengths[ 0 ] -
                                    1 )

                                assign_content = '{}{}'.format(
                                    whitespace, line_tok.value.strip() )
