...
def _AlignGeneric(final_lines, func_is_tok_to_align_current_line,
                  ...
                  func_is_tok_to_align_following_lines,
                  func_get_val_after_align):
  """Implements generic alignement logic which needs to be complemented for
  needed purpose by passing suitable functions as parameters.
  """
  final_lines_index = 0
  while final_lines_index < len(final_lines):
    ...
    for tok in line.tokens:
      ...
      if func_is_tok_to_align_current_line(tok):
        ...
        while not stop_next_it:
          # EOF
          if final_lines_index + len(all_pt_line_lengths) == len(final_lines):
            break

          this_line = final_lines[final_lines_index + len(all_pt_line_lengths)]
          ...
          all_pt_line_lengths.append(pt_line_lengths)

        # Calculate the aligned column value
        max_line_length += 2
        aligned_col = func_calculate_column_to_align_on(tok, max_line_length)
        assert isinstance(aligned_col, int)

        # Update the token values based on the aligned values
        for all_pt_line_lengths_index, pt_line_lengths in enumerate(
            all_pt_line_lengths):
          if not pt_line_lengths:
            continue

          this_line = final_lines[final_lines_index + all_pt_line_lengths_index]

          pt_line_length_index = 0
        ...
        final_lines_index += len(all_pt_line_lengths)
...
def _AlignTrailingComments(final_lines):
  ...
  def _IsTokToAlignCurrentLine(tok):
  ...
  def _CalculatePreTokenLineLengths(this_line, max_line_length):
    ...
    for line_tok in this_line.tokens:
      ...
      if line_tok.is_comment:
        pt_line_lengths.append(len(line_content))
      else:
        line_content += '{}{}'.format(whitespace_prefix, line_tok.value)

    return pt_line_lengths, max_line_length, stop_align_block

  def _CalculateColumnToAlignOn(tok, max_line_length):
    aligned_col = None
  ...
  def _GetValueAfterAlign(line_tok, aligned_col, pt_line_length):
    # Note that there may be newlines embedded in the comments, so we need to
    # apply a whitespace prefix to each line.
    whitespace = ' ' * (aligned_col - pt_line_length - 1)
...
def _AlignAssignment(final_lines):
  ...
  def _CalculatePreTokenLineLengths(this_line, max_line_length):
    ...
    # Only one assignment expression in on each line
    ...
    return pt_line_length, max_line_length, contain_object
