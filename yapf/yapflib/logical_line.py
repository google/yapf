...
def _SpaceRequiredBetween( left, right, is_line_disabled ):
    ...
    lval = left.value
    rval = right.value
    if ( left.is_pseudo and _IsIdNumberStringToken( right ) and left.previous_token and
         _IsIdNumberStringToken( left.previous_token ) ):
    ...
    if not is_line_disabled and ( left.OpensScope() or right.ClosesScope() ):
        if ( style.GetOrDefault( 'SPACES_AROUND_DICT_DELIMITERS', False ) and
             ( ( lval == '{' and
                 _IsDictListTupleDelimiterTok( left, is_opening = True ) ) or
               ( rval == '}' and
...
def _MustBreakBefore( prev_token, cur_token ):
    ...
    if prev_token.is_comment or ( prev_token.previous_token and prev_token.is_pseudo and
